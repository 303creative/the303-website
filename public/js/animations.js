...
function initHero() {
  ...
  const heroItems = ['.rh1', '.rh2', '.rh3', '.rh4', '.rh5', '.rh6', '.rh7'];
...
function initCursor() {
  ...
  window.addEventListener('mousemove', e => {
    ...
    mouseY = e.clientY;
    // Dot snaps instantly
    gsap.to(dot, { x: mouseX, y: mouseY, duration: 0.06, ease: 'none', overwrite: 'auto' });
    // Ring follows with soft lag
    gsap.to(ring, { x: mouseX, y: mouseY, duration: 0.3, ease: 'power3.out', overwrite: 'auto' });
  });
...
function initHeadingReveal() {
  ...
  headings.forEach(h2 => {
    ...
    // Split into words preserving HTML tags
    const wrapped = text.replace(/([\w,.!?'"\u00C0-\u024F]+)/g, '<span class="word-wrap"><span class="word">$1</span></span>');
